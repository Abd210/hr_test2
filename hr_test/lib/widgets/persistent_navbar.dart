...
class _PersistentNavbarState extends State<PersistentNavbar>
  ...
  // Define navigation items
  final List<_NavItemData> _navItems = [
    _NavItemData(icon: Icons.dashboard), // 0
    _NavItemData(icon: Icons.domain),        // 1
    _NavItemData(icon: Icons.people),       // 2
    _NavItemData(icon: Icons.assessment),   // 3
  ];
  ...
  Widget build(BuildContext context) {
    return Container(
      width: 85, // Increased width to accommodate labels
      color: primaryDarkGreen,
      child: Column(
        children: [
          const SizedBox(height: 24),
          ...
          GestureDetector(
            ...
            child: _NavItem(
              data: _NavItemData(icon: Icons.exit_to_app),
              isSelected: false,
              labelAnimation: const AlwaysStoppedAnimation(0.0),
            ),
          ),
...
class _NavItemData {
  ...
  _NavItemData({required this.icon});
}
